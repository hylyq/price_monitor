...
**已知设计取舍：**
- Agent 内置多轮工具调用强制检测：当 LLM 返回文本但未调用任何工具时，自动追加提示要求调用工具获取真实数据，最多推动 2 次。2 次推动后若模型仍然拒绝调用工具，**直接拒绝返回**——返回错误提示引导用户使用 `/pm` 确定性命令。防止模型从训练数据中编造价格或规则（针对 DeepSeek flash 模型可能在多轮对话中持续跳过工具调用、直接输出虚构价格或虚假告警规则的已知行为）
...
| 默认模型 | `deepseek-v4-flash` | DeepSeek 最新主力模型（`deepseek-chat` 将于 2026/07/24 废弃）；配合首轮工具调用强制检测后 tool_use 表现稳定 |
