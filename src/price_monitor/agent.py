...
4. **🚫 严禁主动添加规则**——除非用户的话语中明确包含"监控""盯着""告警""通知"
   "提醒""帮我盯""告诉我"等字眼，否则绝对不得调用 add_price_alert 或
   add_change_alert。查询行情、价格、走势、波动时，**只调用查询工具**
   （get_current_price, get_ticker_detail, calculate_volatility, get_price_history），
   不要添加任何告警规则。用户只是问行情，不等于要求设置监控。

5. **🚫 严禁编造价格**——你训练数据中的价格是过时的、错误的。
   任何涉及当前价格、行情、市场状况的查询，**必须**调用 get_current_price
   或 get_ticker_detail 获取实时数据。**永远不要**从训练数据中输出价格数字。
   违反此规则会导致用户收到虚假行情信息，这是不可接受的。

6. **🚫 严禁报告未查询品种的数据**——你只能报告通过工具实际查询到的品种。
   如果你只调用了 get_current_price("BTC-USDT")，就只报告 BTC 的价格，
   不要提及 ETH、SOL 或其他任何币种的价格——你没有这些数据，
   说出来的必然是训练数据中的过期假数据。
   如果用户问的是泛指的市场行情，使用 get_all_market_prices 一次获取全部数据。
...
如果用户提到的币种你无法确定映射，先调用 get_all_market_prices 查看所有已订阅品种的价格，再询问用户。
...
## 工具选择规则
- "行情如何/市场怎么样/整体走势/现在行情好吗" → get_all_market_prices（泛指市场概览，优先使用）
...
- 工具返回错误时，如实告知并建议替代方案
- 价格始终用 $ 显示
- **只描述你实际查询过的品种**——如果你只查了 BTC，回复中就只提 BTC
- 如果数据不足以回答用户的问题（如用户问 5 个币但你只查到 1 个），
  明确说明你手上有哪些数据、缺少哪些数据，不要用编造的数据补全"""
    ...
    async def answer(self, query: str) -> str:
        ...
        while iteration < MAX_TOOL_ITERATIONS:
            ...
            if text_blocks:
                ...
                # MAX_PUSHES times before refusing to return the text.
                if total_tool_calls == 0:
                    if no_tool_text_count < MAX_PUSHES:
                        ...
                        messages.append({
                            "role": "user",
                            "content": (
                                "你没有调用任何工具。请立即调用对应的工具来获取真实数据。"
                                "不要编造价格或行情信息——你训练数据中的价格是过时的。"
                                "调用工具后我会把结果告诉你，你再基于真实数据回复用户。"
                            ),
                        })
                        continue

                    # MAX_PUSHES exhausted — model is still fabricating, refuse.
                    total_ms = (time.monotonic() - t_start) * 1000
                    logger.warning(
                        "模型在 %d 轮后仍未调用任何工具（已推动 %d 次），拒绝返回可能编造的文本",
                        iteration, MAX_PUSHES,
                    )
                    return (
                        "❌ 未能通过工具获取真实数据。请尝试以下方式：\n"
                        "• 使用 /pm list 查看监控规则\n"
                        "• 使用 /pm price [品种] 查询价格\n"
                        "• 或换一种方式描述您的需求"
                    )
                ...
                # ── Append tool-call summary for transparency ──
                if tool_calls_made:
                    from collections import Counter
                    counts = Counter(tool_calls_made)
                    parts = [f"{name} ×{cnt}" for name, cnt in sorted(counts.items())]
                    combined_text += f"\n\n--- 已调用工具: {', '.join(parts)}"
    ...
    async def answer_with_debug(
        ...
        while iteration < MAX_TOOL_ITERATIONS:
            ...
            if text_parts:
                ...
                # up to MAX_PUSHES times before refusing to return the text.
                tools_called = any(
                ...
                if not tools_called:
                    ...
                    # MAX_PUSHES exhausted — model is still fabricating, refuse.
                    logger.warning(
                        "answer_with_debug: 模型在 %d 轮后仍未调用任何工具（已推动 %d 次），拒绝返回",
                        iteration, MAX_PUSHES,
                    )
                    return (
                        "❌ 未能通过工具获取真实数据。请尝试以下方式：\n"
